...
まずはTime SourceのWorkloadを作成します。`spring.cloud.stream.bindings.output.destination`に宛先を指定します。ここに指定した値がRabbitMQのExchangeになります。
...
`ticktock.time` Exchangeをクリックすると詳細を見ることができます。
...
`ticktock.time.log` Queueをクリックすると詳細を見ることができます。
