...
ただし、翻訳元/先の言語をパラメータ化する方法がわからなかったため、ここでは日->英翻訳専用のテンプレートを作成する例を示します。
...
llama.cppを使うことで、PLaMo翻訳モデルをOpenAI API互換のサーバーとしてローカルで実行できました。 OpenAI APIで翻訳ができれば、いろいろなアプリが翻訳機能を使いやすくなりますね。