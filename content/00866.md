...
summary: この記事では、MicroK8s上のプライベートネットワークにGreenplum on Kubernetes 1.0をリロケートし、OperatorをHelmで導入、クラスターとCommand Centerを構築・検証する手順を紹介します。
...
[Greenplum on Kubernetes 1.0](https://techdocs.broadcom.com/us/en/vmware-tanzu/data-solutions/tanzu-greenplum-k8s/1-0/tgp-on-k8s/index.html)がリリースされたので試してみます。今回はPrivate Network上のMicroK8sにインストールします。
...
今回はPrivate Network上のSelf HostedなContainer Registryにプッシュしてからインストールします。push先はPrivate Network上でないとEULAに違反する恐れがあるので気をつけてください。
...
### Greenplum Command Centerの作成

次にGreenplum Command Centerを作成します。
...
LoadBalancerのEXTERNAL-IPに対して8080ポートで接続します。
