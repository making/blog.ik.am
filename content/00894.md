...
次のコマンドを実行すると、Testcontainersを使ったPostgreSQLと[LGTMスタック](https://hub.docker.com/r/grafana/otel-lgtm)のローカル開発用コンテナが立ち上がります (`src/test/java/com/example/TestcontainersConfiguration.java`を確認してみてください)。DataSourceの設定や、OTLPエンドポイントの設定は自動で行われます。
...
左のメニューのDrilldownから"Traces"を選択します。
...
次に、左のメニューのDrilldownから"Metrics"を選択します。"Prefix filters"で"jdbc"を選択すると、表示をDatasource Micrometerが送信したメトリクスのみに絞ることができます。
...
最後に、左のメニューのDrilldownから"Logs"を選択します。
