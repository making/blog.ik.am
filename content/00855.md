---
title: MicroK8sまたはK3s(というより任意のK8s)でIRSAを使えるようにするメモ
summary: この記事では、MicroK8sまたはK3sでIRSAを有効化する手順を、OIDC Issuer設定からAWS IAMロール付与・テストまで詳しく解説します。
tags: ["Kubernetes", "MicroK8s", "AWS", "OIDC", "IRSA", "K3s"]
categories: ["Dev", "CaaS", "Kubernetes"]
date: 2025-06-18T16:40:05Z
updated: 2025-06-19T00:10:52Z
---
...
EKSでは標準でIRSAが利用できますが、他のKubernetes環境でAPI Serverに追加の設定をすれば利用可能です。この記事では、MicroK8sやK3sでIRSAを有効にする手順を説明します。
...
MicroK8s同様に`/etc/rancher/k3s/config.yaml `でApi Serverの設定を追加します。
