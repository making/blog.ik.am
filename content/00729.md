...
updated: 2026-07-25T12:39:47.422216758Z
---
...
> 
> 従来の形式だけを許可するRoleのままだと、新しく作成したレポジトリからのアクセスが`claim "sub" does not match any associated bound claim values`で失敗します。本記事の[Roleの作成](#Role%E3%81%AE%E4%BD%9C%E6%88%90)を新旧両方の形式に対応するように更新しました。
...
repo:making@106908/redis-adapter-for-spring-session@1311952059:ref:refs/heads/develop
```

新しい形式は、アカウント名やレポジトリ名が再利用された場合に、別人が同じ`sub`を持つトークンを発行できてしまう問題への対策です。区切り文字が`@`なのは、GitHubのアカウント名・レポジトリ名に`@`を使えないからです。

`repo:making/*:ref:refs/heads/main`の`making/`はリテラルなので`making@106908/`にはマッチしません。そのため上記のRoleでは`repo:making@*/*:ref:refs/heads/main`のように`@`付きのパターンも許可しています。こうしておけば、既存のレポジトリと2026年7月15日以降に作成したレポジトリのどちらからでもアクセスできます。
...
`sub_claim_prefix`が`repo:making@106908/...`のようにIDを含んでいれば新しい形式です。既存のレポジトリを新しい形式に切り替えたい場合は、レポジトリまたはOrganizationのOIDC設定からオプトインできます。
