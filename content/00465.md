...
このmanifestではNode Exporterもインストールされてしまうのですが、Node ExporterはBOSH addonとしてインストールした方が
...
前述の通り、Node ExporterはBOSH Addonとしてインストールします。
...
Node Exporter用のscrape configを作成するために、`additional-scrape-configs.yml`に次の内容を記述してください。NodeのIPアドレスはKubernetes Service Discoveryで取得可能です。
...
`additional-scrape-configs.yml`を`Secret`として作成します。
...
このアプリケーションを次のmanifestで`hello` Namespaceにデプロイします。
...
> ConfigMapを変更するだけだと、Podは自動で再作成されないので、更新したい場合は
> 
> ```
