...
```
bosh -n update-runtime-config --name=pks-service-catalog-hack pks-service-catalog-hack.yml 
```
...
これでPKSで作成される全てのk8sクラスタのmaster vm上に上記のhackを適用できます。


少なくともPKS 1.2では動作しますが、1.3以降`bpm.yml`の内容が変わると`sed`の条件が合致しなくなる可能性があります。
