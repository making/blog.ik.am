...
GolangのHello Worldアプリ`git clone`して`cf push`します。初回は各種buildpackのダウンロードがあったりして少し時間がかかります。
...
また、現時点ではHTTPしか受け付けないのとポートもひとつしかバインドできません。
