...
> K8s標準のCronJobではなく、[Furiko](https://furiko.io/docs/execution/jobconfig/sample-configuration)のような拡張スケジューラを使いたい場合もこのClusterConfigTemplateを変更すれば良いです。
...
> `-Dmanagement.health.probes.enabled=false` はSpring Boot Actuatorを使った場合に自動で追加されるProbeの設定を向こうにするために追加しています。
