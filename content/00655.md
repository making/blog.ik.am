...
cert-manager 1.0以上のインストールが必須で、TKG Extensionに含まれている古いバージョンと合わないのでkindで試します。
...
Tanzu Network Registryにログインできます。
...
[Config Tree](/entries/626)を使って`vehicle-db-db-secret`の各項目を`spring.datasource.`以下にバインドされるように次のmanifestをデプロイします。
...
Postgresリソースを作成するとTLS対応が自動で行われます。
...
> ```
> 
### Postgres InstanceのデータをBackupする
