...
Googleが公開したGemma 3をllama-cppで動かし、 OpenAI API経由でアクセスします。また、Spring AIを経由してこのAPIにアクセスし、Function CallingやMCP連携を試します。

実行環境はこちら(Apple M4 Max, CPU 16コア、GPU 40コア、ユニファイドメモリ 128GB)です
...
次のコードのようにシンプルにChat ClientにGETのリクエストパラメータのプロンプトを渡しているだけです。また、長いレスポンス用にPOSTでプロンプトをリクエストボディとして送り、Server Sent Eventでも返せるようにしています。
...
ビルドして実行します。`spring.ai.openai.base-url`プロパティにはllama.cppのサーバーURLを指定します。
...
[OpenAI形式のFunction Calling](https://platform.openai.com/docs/guides/function-calling?api-mode=chat)を[llama.cppはサポートしています](https://github.com/ggml-org/llama.cpp/blob/master/docs/function-calling.md)。
...
ここでは[Spring AIのドキュメントのサンプル](https://docs.spring.io/spring-ai/reference/api/tools.html#_information_retrieval)通りのToolを追加し、現在時刻を教えられるようにします。
...
`bbc.comのニュースヘッドライン3件のタイトルを日本語で教えて`というプロンプトも送ってみます。
...
BBCのニュースヘッドラインは以下の3つです。

1. **ロシアは平和を望んでいない、EU外相が表明、トランプとプーチンが土地問題について協議予定**
2. **「すべてが終わった」：ウクライナ兵士たちがクルスクからの撤退を振り返る**
3. **インド、バングラデシュとパキスタンの関係改善を警戒**
```

MCP Serverから取得した情報を日本語に翻訳することもできました。

Spring AIでは[MCP Server](https://docs.spring.io/spring-ai/reference/1.0/api/mcp/mcp-server-boot-starter-docs.html)を実装することもできるので、LLMを色々な方法で拡張することができそうです。

---

プライベート LLMでもこれだけの機能があれば、色々な場面で活用できそうですね。