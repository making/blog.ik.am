...
### Pinnipedを経由してWorkload Clusterのkubeconfigを取得

Pinnipedを導入するメリットとして、adminではなくユーザーの権限でk8sクラスタにアクセスするためのkubeconfigを生成できます。

ドキュメントは [こちら](https://docs.vmware.com/en/VMware-Tanzu-Kubernetes-Grid/1.3/vmware-tanzu-kubernetes-grid-13/GUID-cluster-lifecycle-connect.html#id-mgmt) .
...
初回はアクセストークンがないため、ブラウザが立ち上がり、ログインフォームが表示されます。LDAP認証の場合は、Dexのログインフォームです。
...
### 任意の端末上からManagement Clusterにtanzu loginする

Pinnipedを導入するもう一つのメリットは、TKG管理者が`tanzu` CLIでmanagement clusterへアクセスにするのに`~/.kube-tkg/config`を持ち込む必要がなくなる点です。
任意の端末上で`tanzu login`を実行して`~/.kube-tkg/config`を取得できます。

ドキュメントは [こちら](https://docs.vmware.com/en/VMware-Tanzu-Kubernetes-Grid/1.3/vmware-tanzu-kubernetes-grid-13/GUID-mgmt-clusters-configure-id-mgmt.html#authenticate-users-on-a-machine-without-a-browser-6]) 。
...
```
✖  failed to login to the management cluster carrot, err-Failed to invoke API on cluster : Get "https://192.168.11.110:6443/version?timeout=1m0s": context deadline exceeded (Client.Timeout exceeded while awaiting headers)
Error: exit status 1
✖  exit status 1 
```
...
You can now access the cluster by running 'kubectl config use-context tanzu-cli-carrot@carrot'
```

```
$ kubectl config use-context tanzu-cli-carrot@carrot
$ kubectl cluster-info
Kubernetes control plane is running at https://192.168.11.110:6443
...
```
tanzu cluster kubeconfig get orange
kubectl config use-context tanzu-cli-orange@orange
```
