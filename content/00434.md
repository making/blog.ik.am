---
title: Cloud Foundryのbuildpackを使ってDockerイメージを作成し、Kubernetesにデプロイ
...
一方、buildpackではrootfsとして[cflinuxfs2](https://github.com/cloudfoundry/cflinuxfs2)のUbuntu14.04の派生のイメージを使用します。
定期的にセキュリティパッチがあてられています。また各言語ごとにランタイム/ミドルウェアがメンテナンスされており、原則としてCVE（Common Vulnerabilities and Exposures）公開後48時間以内にパッチがリリースされます。
...
[CF Local](https://github.com/sclevine/cflocal)Pluginを使用することで、前述のbuildpackによるメリットをDockerイメージ作成にも適用できます。
CF LocalはLaptop内で`cf push`をエミュレートするプライグんですが、DockerイメージへのExportに対応しています。
...
また、このフローにおいてはCloud FoundryのランタイムもDockerデーモンも不要ですので、CI/CD化も容易です。
...
Tomcatが8.5系に上がりました。また、JVMのMemory Calculatorの仕様も[変わりました](https://www.cloudfoundry.org/just-released-java-buildpack-4-0/)。
...
CF Local Pluginを使って、`Dockerfile`を作ることなくBuildpackからDockerイメージを作成し、k8sにデプロイできることを確認しました。
