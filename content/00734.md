...
`DD_ENTITY_ID`と`DD_AGENT_HOST`が自動で追加されていることがわかります。`DD_AGENT_HOST`にはhostIPが設定されています。そのため、ログに`"agent_url":"http://172.19.0.2:8126"`が出力されていました。
...
> ```
> 
> この場合は `--env DD_AGENT_HOST=datadog-agent.datadog.svc.cluster.local` になります。
...
> どうしてもYAMLを使いたくない場合は、`--anotation`オプションの代わりに`--param-yaml`オプションを使って次のコマンドでも設定可能です
> 
> ```
> tanzu apps workload apply rest-service \
>   --app rest-service \
