...
このbuildpackを使うだけでは実際のデータをDatadogに送ることはできません。

Datadogにデータを送るには次のドキュメントに従ってData AgentをKubernetesにインストール必要があります。

* https://docs.datadoghq.com/containers/kubernetes/installation/?tab=helm


本記事ではDatadog Agentをインストールし、TAPにデプロイしたWorkloadをモニタリングしてみます。
...
`helm template`コマンドを使ってmanifestを生成し、`kubectl apply`でデプロイします。
