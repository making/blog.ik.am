...
CASSANDRA_PORT=$(docker inspect demo-scylla-cassandra-1 | jq -r '.[0].NetworkSettings.Ports["9042/tcp"][0].HostPort')
java -jar target/demo-scylla-0.0.1-SNAPSHOT.jar --spring.cassandra.contact-points=localhost:${CASSANDRA_PORT} --spring.cassandra.local-datacenter=datacenter1
```
