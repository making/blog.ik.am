...
まずはSpring BootアプリケーションでTLSを有効にするために、`application.properties`を修正します。
`hello-pks-config.yml`に次の内容を記述して下さい。
...
data:
  application.properties: |-
      ...
      server.ssl.key-store=file:///keystores/hello-pks.jks
      server.ssl.key-store-type=PKCS12
      server.ssl.key-alias=hello-pks
```

アプリケーションのポートは慣習的に`8443`にします。
SecretにはPEM形式の証明書が保存されていますが、JavaアプリケーションではPEM形式のままでは読み込めず、
PKCS12形式に変換した後、Keystoreにインポートする必要があります。この変換処理は後ほど記述します。
...
data:
  ...
  spring.security.user.password: WTRueXM3ZjEx
  # ここから追加
  server.ssl.key-password: Y2hhbmdlbWU= # echo -n changeme | base64
```

Serviceの設定を変更し、Podの8443番ポートにルーティングするようにします。
`hello-pks.yml`に次の内容を記述して下さい。
...
最後に、Podの設定を変更し、TLS証明書をKeystoreにインポートして、コンテナにマウントしてアプリケーションから参照できるようにします。
...
`hello-pks`コンテナはポート番号を`8080`から`8443`に変える必要があります。また、ヘルスチェックのプロトコルもHTTPSにする必要があります。
