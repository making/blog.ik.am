---
title: Cloud Foundry(v241)のDiegoオンリー構成をBOSH Liteでデプロイ
tags: ["Cloud Foundry", "BOSH", "BOSH-Lite", "Diego"]
categories: ["Dev", "PaaS", "CloudFoundry"]
---

今となってはDiegoオンリー構成が当たり前でDEAが入っているのはレガシー。Pivotal Cloud FoundryでもすでにDiegoオンリー構成になっているけれど、
[BOSH LiteでCloud Foundryをインストールするドキュメント](http://docs.cloudfoundry.org/deploying/boshlite/index.html)が未だにDEA構成でインストールしてからDeigoを入れる形になっている。これでもアプリは問題はないけれど、使わないインスタンスがいるので気持ち悪い。
ググってもDiegoオンリーのマニフェストがないなーと思っていたら、普通に[cf-releaseプロジェクトにあった] (https://github.com/cloudfoundry/cf-release/tree/master/example_manifests)([2016年8月にDEAとHM9000が削除されていた](https://github.com/cloudfoundry/cf-release/commit/cfcf092bec7d0603653a0c408d80327b134333fb#diff-61a813f43a9b8d1fe97af8955b760b94))。
...
[`minimal-aws.yml`](https://github.com/cloudfoundry/cf-release/blob/b2a7c0514675341c90de349bedf52626d9d16e28/example_manifests/minimal-aws.yml)を参考に、bosh-lite版のmanifestを作成。
...
ログは[こちら](https://gist.github.com/making/8ed9c10662695ab4f8d00da8b6449edd)。
コンパイル済みパッケージが使われており、たった10分でデプロイできた。今までの([DEA版](https://blog.ik.am/entries/368), [Diego版](https://blog.ik.am/entries/370))はなんだったんだ。
...
🔼のマニフェストだと`admin` / `PASSWORD`でログインできる。
...
`cf.yml`の次の数字を変更して、`bosh -n deploy`
...
アプリ側で環境変数`CF_INSTANCE_IP`を表示するようにして、`cf scale <app> -i 5`とかやると2,2,1で配置されていることがわかるはず。
