...
その後、「Generate Project」をクリックす
るとdemo.zipがダウンロードされます。zipを展開して、MavenプロジェクトをIDEにインポートしてください。
...
↑の図では、必要であるにもかかわらず対応するSQLがないメソッドは赤色で表示されます。このように表示させるにはIntelliJ IDEの[Doma Supportプラグイン](https://github.com/siosio/DomaSupport)をインストールする必要があります。Doma 2を使用するためにプラグインは必須ではありませんが、あると便利です。
