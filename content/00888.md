...
1. 同じ `syslog.program`、`syslog.hostname`などを持つログを同一リソースにグループ化
2. `syslog.program` などを `log.attributes` から `resource.attributes` に昇格

これにより、異なるプログラムからのログが同じバッチで処理されても、リソース属性が混在しません。

次のProcessor(`transform/syslog-resource`)でResource Attributesの設定を行いますが、ここの`groupbyattrs` がないと、例えば、以下の問題が発生します：

1. 複数のログが同じバッチで処理される
2. 各ログで `resource.attributes["service.name"]` が上書きされる
3. 最後に処理されたログの値で全ログの `service.name` が決まってしまう

Resource Attributesに設定したいキーは全て `groupbyattrs` の `keys` に含めてください。
...
繰り返しになりますが、これらのResource Attributesに設定したいキーは全て `groupbyattrs` の `keys` に含めてください。
...
![image](https://qiita-image-store.s3.ap-northeast-1.amazonaws.com/0/1852/ea7da897-cd57-4905-bbdc-0ed34186d271.png)
