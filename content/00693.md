...
> ℹ️ `pull_from_kp_default_repo`を`false`にすることでTBS関連のイメージをTanzu Netのregistryからpullできる。
> `pull_from_kp_default_repo`が`true`の場合は、`kp_default_repository`に指定したregistryからpullするので、事前に`imgpkg`コマンドでrelocationしておく必要がある。
