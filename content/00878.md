...
[こちらの記事](/entries/872)でJava 22で正式版となったForeign Function &
Memory（FFM）APIを使用して、Rustで実装した関数をJavaから呼び出すサンプルを試しましたが、
...
題材は前回と同じく、[竹内関数](https://ja.wikipedia.org/wiki/%E7%AB%B9%E5%86%85%E9%96%A2%E6%95%B0)
を使いました。
...
インストールが完了すると、`~/.moon/bin`にMoonBitのツールチェーンが配置されます。インストールスクリプトは自動的に
`~/.zshrc`にPATHを追加してくれます。
...
ブラウザが開き、GitHubでの認証が求められます。認証が完了すると、GitHubのユーザー名（例: `making`
）がMoonBitのユーザー名として使用されます。
...
fibonacci関数のサンプルコードは次のように、`loop`構文で末尾再帰最適化をしているように見える。
