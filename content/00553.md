...
Readiness Probeはコンテナが応答できるかどうかをチェックし、Liveness Probeはコンテナが起動しているかどうかをチェックします。
...
> Spring Boot Actuatorのhealthエンドポイントは利用しているデータベースのヘルスチェックなども行いますが、データベースがダウンしている際にコンテナを再起動しても意味がない場合は、Rediness Probeではデーターベースなどのチェックは敢えて行わなず`path`を`/actuator/info`に変えることも考えられます。
