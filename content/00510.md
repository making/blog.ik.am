...
[vanilla-spring-webflux-fn-blank](https://github.com/making/vanilla-spring-webflux-fn-blank)の`0.2.10`でプロジェクトを作成した場合はあらかじめNative Imageビルド用の設定が用意されています。
...
> その場合は`src/main/resources/META-INF/native-image/com.example.moneyger/framework.json`のJSON配列に次を追加すれば良いです。
> 
> ```json
>   {
...
`src/main/resources/META-INF/native-image/com.example.moneyger/app.json`で次のように定義します。
