...
今回は外部Vaultサーバーとして、[Cloud FoundryにデプロイしたVault](https://blog.ik.am/entries/423)を利用する。月100円以下(無料期間あり)で管理できるのでおすすめ。
...
```
vault write auth/approle/role/concourse secret_id_ttl=10m token_num_uses=10 token_ttl=20m token_max_ttl=30m secret_id_num_uses=40
```
...
Concourse用AppRoleにポリシーを適用

```
vault write auth/approle/role/concourse secret_id_ttl=10m token_num_uses=10 token_ttl=20m token_max_ttl=30m secret_id_num_uses=40 policies=concourse
```
...
> 403エラー(permission denied)が出る場合は、ポリシーが設定されていないか、ATCに設定したSECRET IDが間違っている可能性がある。
