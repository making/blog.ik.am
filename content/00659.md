...
Chartで設定できる項目だけだと、どうしてもうまく動かない箇所(ブラウザとCLIで両方ログイン成功させることができない)があったので、 [Post Rendering](https://helm.sh/docs/topics/advanced/#post-rendering) 機能を使って、yttでmanifestを少しカスタマイズします。
