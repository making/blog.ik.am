...
テストなどで、固定の時刻を返却したい場合は、次のように`InstantSource`を上書きできます。
...
`Instant`を作成したい場合は、`InstantSource`をinjectして利用します。
...
エンタープライズ開発でよく見られるのはシステム時刻をデータベースから取得するケースです。結合テストなどで、特定の時間のテストを行いたい場合などに有効です。
...
JDK 17以降であれば、タイムゾーンを含まないシンプルな`InstantSource`の利用を検討してください。タイムゾーンを扱う必要がある場合は`withZone`で`Clock`に変換するか、最初から`Clock`を使うことも選択肢です。
DIコンテナと組み合わせることで、本番環境ではシステムクロック、テスト環境では固定時刻やデータベースからの取得など、柔軟な切り替えが可能になります。