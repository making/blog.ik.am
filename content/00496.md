...
Vaultは機能豊富ですが、ConcourseのCredentials管理のみのために使いにはtoo muchなのと、Storage Backendとしてofficialにサポートされているのが、Consul、Filesystem、In-Memoryだけなので、
...
* atcのCredHub連携の設定情報は[https://github.com/concourse/concourse/blob/v3.8.0/jobs/atc/spec#L486-L509](こちら)です。
...
```
uaac target https://(UAAのIP):8443 --skip-ssl-validation
...
このACL機能を有効にするには`credhub` jobの`properties`に[https://github.com/pivotal-cf/credhub-release/blob/1.6.5/jobs/credhub/spec#L167-L169](credhub.authorization.acls.enabled)を`true`にする必要があります。
...
ここに`atc_to_credhub`クライアントがアクセスできるように設定します。
