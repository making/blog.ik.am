...
Long Running Processではなく、[Task](https://docs.cloudfoundry.org/devguide/using-tasks.html)として実行したいので`cf push`でコンテナイメージができたあとは`cf stop`ですぐにコンテナを止めます。
...
`cf logs hello-batch --recent`でログを確認すると、起動時にJobが実行されていないことがわかります。これは`spring.batch.job.enabled`が`false`だからです。
...
`cf logs hello-batch`でログを確認すると、今度はJobが実行されていることが確認できます。
