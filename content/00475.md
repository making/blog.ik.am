...
```
cf curl /v2/apps/$(cf app demo-micrometer --guid) | jq -r .entity.detected_start_command
```
...
```
cf run-task hello-batch "$(cf curl /v2/apps/$(cf app demo-micrometer --guid) | jq -r .entity.detected_start_command) --spring.batch.job.enabled=true"
```
...
```
cf create-job hello-batch hello-job "$(cf curl /v2/apps/$(cf app demo-micrometer --guid) | jq -r .entity.detected_start_command) --spring.batch.job.enabled=true"
```
