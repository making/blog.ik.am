...
`ops_manager_image_uri`は[https://network.pivotal.io/products/ops-manager](https://network.pivotal.io/products/ops-manager)の`Pivotal Cloud Foundry Ops Manager for Azure`から取得可能です。2.4の最新版のイメージを取得するのがオススメです。

`terraform``env_name`、`env_short_name`(`-`を含めてはいけない)、`dns_suffix`、`location`は好きなように変更してください。
