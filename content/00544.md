...
次の`value.yml`に設定項目を記述します。`#@data/values`アノテーションが必要です。
...
`ytt`ライブラリの`data`モジュールを使って読み込みます。`#@data/values`アノテーション
...
JSONを読み込む例も紹介します。別のツールの結果がJSONとして出力され、それをそのまま読み込みたいケースがあります。
...
切り出したTaskファイルを`hello.lib.yml`というファイルに保存します。読み込まれた際にマルチドキュメントなYAMLとして出力されないようにファイル名に`.lib`を含めます。
...
この`hello.lib.yml`を`data`モジュールで文字列としてとして読み込み、`yaml`モジュールでdecodeし、`config`に設定します。
...
---
YTTの基本を見てきました。[次の記事](/entries/545)はYAMLの柔軟な加工に便利なOverlayを見ます。
