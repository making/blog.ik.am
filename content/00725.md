...
tags: ["Kubernetes", "Cartographer", "kind", "Tanzu", "TAP", "Knative", "kapp-controller", "ytt"]
...
ここまで定義したApp CR群自体はGitOpsで管理するためのApp CRの定義。
10分間隔でgit pullします。
...
1. kapp-controllerをインストール
1. Service Accountを作成
1. credentialsのSecretの登録 (sopsを使用する場合は、[GPG Private Key](https://carvel.dev/kapp-controller/docs/v0.43.2/sops/#using-gpg-2)の登録)
1. apps App CRの作成
