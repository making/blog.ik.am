---
title: BOSH CLI 2でConcourseを自宅サーバーのBOSH Lite on Virtual Box上インストールする
...
BOSH CLI 2の機能でpasswordやTLS証明書といったcredentials情報は自動生成させ、manifest上には記述しない。
...
`creds-concourse.yml`に先ほど生成した証明書が既に設定されているため、再生成するために、`creds-concourse.yml`から`atc_ssl.certificate`と`atc_ssl.private_key`、`atc_ssl.ca`を削除する。
