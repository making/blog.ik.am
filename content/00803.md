...
LocalでLLMを試すのに[Ollama](https://ollama.com/)が人気です。
Spring AIには[Ollama用のChat Client](https://docs.spring.io/spring-ai/reference/api/clients/ollama-chat.html)が用意されていますが、
Ollamaには[OpenAI API互換API](https://ollama.com/blog/openai-compatibility)も用意されているので、OpenAIへの切り替えも想定して、[OpenAI用のChat Client](https://docs.spring.io/spring-ai/reference/api/chat/openai-chat.html)を使ってOllamaにアクセスしてみます。
...
このアプリ自体はOpenAI向けのアプリですが、プロパティを変えるだけでOllama経由で様々なモデルを扱えるのが今回の方式の利点ですね。
...
同様のことは[llama-cpp-python](/tags/llama.cpp/entries)でも行えましたが、Ollamaの方がモデルのダウンロードが簡単です。
