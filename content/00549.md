---
title: QuarksSecretでKubernetesのSecretをサーバーサイドで自動生成させる
tags: ["Kubernetes", "Quarks"]
categories: ["Dev", "CaaS", "Kubernetes", "Secret", "Quarks"]
---

Kubernetesのmanifestをgitで管理するとき、`Secrets`の扱いが課題になりがちです。
...
gitで管理するアプリケーションのマニフェストファイルのサンプルを
