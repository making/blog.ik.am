---
title: Spring Batch 6 / Spring Boot 4ではデフォルトでDB不要になっている
summary: この記事では、DB不要のSpring Batch 6 / Spring Boot 4でHello Worldジョブを作成し、GraalVMネイティブイメージで実行する方法を紹介します.
tags: ["Spring Batch", "Spring Boot", "Java", "Spring"]
categories: ["Programming", "Java", "org", "springframework", "batch"]
date: 2026-01-24T01:41:45.202739435Z
updated: 2026-01-24T01:41:45.202739435Z
---

[以前の記事](/entries/847)でSpring Batch 5 / Spring Boot 3系でDBを使わないSpring Batchジョブの起動方法について説明しましたが、[Spring Batch 6 / Spring Boot 4では**デフォルトでDB不要**](https://spring.io/blog/2025/07/23/spring-batch-6#resourceless-batch-infrastructure-by-default)になっています。
...
次のようにJobを定義します。Tasklet指定時に`PlatformTransactionManager`を設定しない場合に`ResourcelessTransactionManager`が使われるようになりました。
