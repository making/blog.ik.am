...
echo 'export LDFLAGS="-L/opt/homebrew/opt/libpq/lib"' >> ~/.zprofile
echo 'export CPPFLAGS="-I/opt/homebrew/opt/libpq/include"' >> ~/.zprofile
source ~/.zprofile
```
...
```
echo 'export PATH="/opt/homebrew/opt/kubernetes-cli@1.30/bin:$PATH"' >> ~/.zprofile
source ~/.zprofile
```
...
```
echo "alias ll='ls -laGh'" >> ~/.zprofile
cat <<'EOF' >> ~/.zprofile
precmd () {
  psvar=()
