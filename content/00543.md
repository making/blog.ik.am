...
<img src="https://user-images.githubusercontent.com/106908/90330477-e7588900-dfe7-11ea-86a6-91533b0b2fa1.png" width="300" />

Cluster APIについてより知りたい場合は次のKubeAcademyの動画を見ると良いです。
...
TKGで作成されるBastion Server用のSSH Keyを作成します。
...
次のコマンドでManagement Clusterを作成します。
...
次のコマンドでWorkload ClusterとManagement Clusterを削除します。

```
tkg delete cluster demo -y
tkg delete management-cluster aws-mgmt-cluster -y
```
...
`tkg`コマンドでAWSでもvSphereでも同じようにクラスタを作成・管理できるため、
